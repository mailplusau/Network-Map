...
function summary_page(request, response) {
  if (request.getMethod() === "GET") {
    ...
    var form = nlapiCreateForm('First Mile Suburb Selection');
    ...
    if (role != 1000) {
      ...
      resultSet_zee.forEachResult(function(searchResult_zee) {
        ...
        if (request.getParameter('zee') == zeeid) {
          inlineQty += '<option value="' + zeeid + '" selected="selected">' +
            zee_name + '</option>';
          zee = zeeid;
        } else {
          inlineQty += '<option value="' + zeeid + '">' + zee_name +
            '</option>';
        }
    ...
    if (!isNullorEmpty(zee)) {
      ...
      var network_json = partner_record.getFieldValue(
        'custentity_sendle_recovery_suburbs');
    ...
    partner_record.setFieldValue('custentity_sendle_recovery_suburbs_main',
      code_array);
    partner_record.setFieldValue('custentity_sendle_recovery_suburbs',
      network_JSON);
