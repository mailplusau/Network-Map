 ...
 * @Last Modified time: 2020-11-30 10:59:48
 *
 */
...
function pageInit() {
	...
	console.log(date_from_iso)
	console.log(date_to_iso)

	if (!isNullorEmpty(date_from_iso) && !isNullorEmpty(date_to_iso)) {

		$('#date_from').val(date_from_iso);
		$('#date_to').val(date_to_iso);
		var from_date = new Date(date_from_iso);
		var to_date = new Date(date_to_iso);
	...
	} else if (isNullorEmpty(date_from_iso) && !isNullorEmpty(date_to_iso)) {
	...
	if (!isNullorEmpty(sendle_tasks) && sendle_tasks.length > 0) {
		...
		for (var x = 0; x < sendle_tasks.length; x++) {
			...
			if (sendle_tasks[x]['pickupTask'] === true) {
				var extraBarcodes = '';
				var status = 'FC';
				var timeCreated = sendle_tasks[x]['timeCreated'];
				if (!isNullorEmpty(timeCreated)) {
					var dateObject = new Date(timeCreated);
					var humanCreatedDateFormat = dateObject.toLocaleString();
				} else {
					var humanCreatedDateFormat = "";
				}
				...
				if (!isNullorEmpty(completionTime)) {
					const milliseconds = completionTime;
				...
				if (!isNullorEmpty(scannedBarcodes)) {
					...
					totalBarcodesCount = scannedBarcodesArray.length;
					if (totalBarcodesCount > 3) {
						extraBarcodesCount = totalBarcodesCount - 3;
					}
					...
					if (pickupTask === true) {
						...
						pickupBarcodesArray = pickupBarcodesArray.filter(function(el) {
							return el != "";
						});
						...
						if (scannedBarcodesArray.length > pickupBarcodesArray.length || scannedBarcodesArray.length == pickupBarcodesArray.length) {
							status = 'C';
						} else if (scannedBarcodesArray.length < pickupBarcodesArray.length) {
							status = 'PFC';
						}
				...
				dataSet += '{"job_date":"' + humanCreatedDateFormat + '",';
				dataSet += '"shortId":"' + shortId + '",';
				dataSet += '"pickupTask":"' + pickupTask + '",';
				dataSet += '"driverName":"' + driverName + '",';
				dataSet += '"teamName":"' + teamName + '",';
				dataSet += '"status":"' + status + '", ';
				dataSet += '"onDemandPickup":"1", ';
				dataSet += '"onDemandPickupRate":"6.00", ';
				dataSet += '"onDemandPickupAdditionalRate":"1.50", ';
				dataSet += '"onDemandPickupDispatchRate":"1.00", ';
				dataSet += '"totalAdditionalItem":"' + parseFloat(1.50 * extraBarcodesCount) + '", ';
				dataSet += '"amount":"' + parseFloat(6.00 + (1.50 * extraBarcodesCount)) + '", ';
				dataSet += '"totalAmount":"' + parseFloat(7.00 + (1.50 * extraBarcodesCount)) + '", ';
				dataSet += '"totalBarcodesCount":"' + totalBarcodesCount + '", ';
				dataSet += '"extraBarcodesCount":"' + extraBarcodesCount + '", ';
	...
	$(document).ready(function() {
		...
		table = $("#customer").DataTable({
			...
			"columns": [{
				...
				"data": "totalBarcodesCount" //TOTAL PICKUPS COUNT
			}, {
				"data": null,
				"render": function(data, type, row) {
						return '<div style="word-break: break-all;">' + data.onDemandPickup + '</div>';
					} //ON DEMAND PICKUP (1-3 ITEMS)
			}, {
				"data": null,
				"render": function(data, type, row) {
						return '<div style="word-break: break-all;">$' + data.onDemandPickupRate + '</div>';
					} //ON DEMAND PICKUP RATE
			}, {
				"data": "extraBarcodesCount" //ON DEMAND PICKUP ADDITIONAL (3+ ITEMS)
			}, {
				"data": null,
				"render": function(data, type, row) {
						return '<div style="word-break: break-all;">$' + data.onDemandPickupAdditionalRate + '</div>';
					} //ON DEMAND PICKUP ADDITIONAL RATE
			}, {
				"data": null,
				"render": function(data, type, row) {
						return '<div style="word-break: break-all;">$' + data.totalAdditionalItem + '</div>';
					} //TOTAL ADDITIONAL ITEM
			}, {
				"data": null,
				"render": function(data, type, row) {
						return '<div style="word-break: break-all;">$' + data.onDemandPickupDispatchRate + '</div>';
					} //ON DEMAND PICKUP DISPATCH RATE
			}, {
				"data": null,
				"render": function(data, type, row) {
						return '<div style="word-break: break-all;">$' + data.amount + '</div>';
					} //AMOUNT
			}, {
				"data": null,
				"render": function(data, type, row) {
						return '<div style="word-break: break-all;">$' + data.totalAmount + '</div>';
					} //TOTAL AMOUNT
			}, {
				"data": "status"
			}],
			...
			"footerCallback": function(tfoot, data, start, end, display) {
				//Get data here 
				console.log(data);
				var totalScannedBarcodes = 0;
				var totalExtraScannedBarcodes = 0;
				var totalAmount = 0.0;
				var totalAmount2 = 0.0;
				//Do whatever you want. Example:
				var api = this.api();
				for (var i = 0; i < data.length; i++) {
                    totalScannedBarcodes += parseInt(data[i]['totalBarcodesCount']);
                }
                for (var i = 0; i < data.length; i++) {
                    totalExtraScannedBarcodes += parseInt(data[i]['extraBarcodesCount']);
                }
                for (var i = 0; i < data.length; i++) {
                    totalAmount += parseFloat(data[i]['amount']);
                }
                for (var i = 0; i < data.length; i++) {
                    totalAmount2 += parseFloat(data[i]['totalAmount']);
                }
                console.log(totalScannedBarcodes);
				$(api.column(5).footer()).html(totalScannedBarcodes);
				$(api.column(8).footer()).html(totalExtraScannedBarcodes);
				$(api.column(12).footer()).html(totalAmount);
				$(api.column(13).footer()).html(totalAmount2);
			}
		});
	});
}
...
function reloadPageWithParams() {
	...
	var upload_url = baseURL + nlapiResolveURL('suitelet', 'customscript_sl_sendle_invoicing', 'customdeploy_sl_sendle_invoicing') + '&custparam_params=' + encodeURIComponent(params);
